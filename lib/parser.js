...
function parseText(text, referenceCaps, verseCaps) {
...
//			var match;
		...
		if (_.size(part) == 1) {
			...
			part = {
				reference: (referenceCaps) ? part[0].toUpperCase() : part[0],
				verse: (verseCaps) ? _.slice(_.values(part), 1).join(' ').toUpperCase() : _.slice(_.values(part), 1).join(' ')
			};
...
function createVerse(text, number) {
	...
	return {
		...
		text: words.length > 6
			? words.slice(0, -4).join(' ') + ' ' + words.slice(-4).join('\xA0')
			: words.join(' ')
	};
}
